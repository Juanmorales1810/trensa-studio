    ...
    return (
        <div className="grid auto-rows-[140px] grid-cols-3 gap-1 w-full pb-4 md:auto-rows-[192px] md:gap-4">
            {contenido.map((cont, i) => (
                ...
                >
                    <Image src={cont.image} alt={cont.title} width={800} height={400} className="rounded-none object-cover h-full opacity-25 saturate-0 group-hover:opacity-100 group-hover:saturate-100 transition-opacity" />
                </div>
            ))}
        </div>
    )
